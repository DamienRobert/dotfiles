...
if &cp || (exists('g:loaded_myplugins') 
      \ && !exists('g:force_reload_vim_myplugins'))
  finish 
endif
...
function! TransposeWords(type, cursor_pos, ...)
...
" Commands: {{{1
command! -bar -count=1 TransposeWords call TransposeWords('next','follow',<Count>)
command! -bar -count=1 TransposeWordsBack call TransposeWords('prev','follow',<Count>)
